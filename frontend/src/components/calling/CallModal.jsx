...
export default function CallModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6 bg-zinc-950/75 backdrop-blur-md animate-fade-in">
      <div
        className={`relative w-full overflow-hidden rounded-[32px] border border-border bg-card shadow-2xl transition-all ${
          isVideo ? 'max-w-3xl aspect-[4/3] md:aspect-[16/10] max-h-[85vh]' : 'max-w-md p-8'
        }`}
      >
        {/* Top bar controls */}
        ...
        {isVideo ? (
          <div className="relative size-full bg-zinc-950 flex items-center justify-center">
            ...
            <div className="absolute top-4 left-4 z-10 w-28 md:w-36 aspect-[3/4] overflow-hidden rounded-2xl border-2 border-white/20 bg-zinc-900 shadow-xl">
              {!isVideoOff && localStream ? (
                ...
                <div className="grid size-full place-items-center bg-zinc-800 text-zinc-400 text-xs text-center p-2">
                  <svg className="size-6 mb-1 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <line x1="2" x2="22" y1="2" y2="22" />
                    <path d="m16 16-3.5-2" />
                    <path d="m15 10 4.553-2.276A1 1 0 0 1 21 8.618v6.764a1 1 0 0 1-1.447.894L15 14v-4z" />
                    <rect width="14" height="12" x="1" y="6" rx="2" />
                  </svg>
                  <span>{t('calling.turnOffCamera', { defaultValue: 'Kamera kapalı' })}</span>
                </div>
              )}
            </div>
            ...
            <div className="absolute bottom-6 inset-x-0 z-20 flex items-center justify-center gap-4">
              ...
              <button
                type="button"
                onClick={onToggleMute}
                className={`grid size-12 place-items-center rounded-full backdrop-blur-md transition cursor-pointer ${
              ...
              <button
                type="button"
                onClick={onEndCall}
                className="grid size-14 place-items-center rounded-full bg-rose-600 text-white shadow-lg shadow-rose-600/40 hover:bg-rose-700 transition-transform active:scale-95 cursor-pointer"
                title={t('calling.endCall', { defaultValue: 'Görüşmeyi Sonlandır' })}
              >
            ...
            {/* Time badge */}
            {isConnected ? (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 rounded-full bg-black/40 px-3.5 py-1 text-xs font-medium text-white backdrop-blur-md">
