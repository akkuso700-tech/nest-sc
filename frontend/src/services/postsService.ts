...
async function readDirectUploadResponse(response: Response, fallbackMessage: string): Promise<any> {
  ...
  if (!response.ok) {
    const error = new Error(payload?.message || fallbackMessage) as any
...
async function fetchUploadAction(endpoint: string, action: string, options: RequestInit = {}): Promise<Response> {
  const separator = endpoint.includes('?') ? '&' : '?'
  return fetch(`${endpoint}${separator}action=${encodeURIComponent(action)}`, options)
}

async function resolveRemoteOffset(endpoint: string, ticket: string): Promise<any> {
...
function getInitialChunkSize(maxAllowedBytes: number = 8 * 1024 * 1024): number {
  const serverMax = Math.max(512 * 1024, Number(maxAllowedBytes) || 8 * 1024 * 1024)
  const nav = typeof navigator !== 'undefined' ? navigator as any : null
  const connection = nav ? nav.connection || null : null
...
function adaptNextChunkSize(currentSize: number, durationMs: number, maxAllowedBytes: number): number {
...
export interface UploadLoopVideoOptions {
  signal?: AbortSignal
  onProgress?: (percent: number) => void
}

export async function uploadLoopVideoDirect(
  file: File,
  options: UploadLoopVideoOptions = {}
): Promise<{ ticket: string; sourceUrl: string } | null> {
  let session: any
  try {
...
export function getFeed(params: GetFeedParams = {}): Promise<FeedResponse> {
  ...
  return apiRequest<FeedResponse>(`/posts/feed${query ? `?${query}` : ''}`)
}

export function getTrendingTopics(params: { limit?: number } = {}): Promise<Array<{ tag: string; count: number }>> {
  const searchParams = new URLSearchParams()
  ...
  return apiRequest<Array<{ tag: string; count: number }>>(`/posts/trends${query ? `?${query}` : ''}`)
}

export function createPost(payload: FormData | Record<string, any>): Promise<{ post: Post }> {
...
export function getPostDetail(postId: string): Promise<{ post: Post }> {
...
export function registerPostView(
  ...
  return apiRequest<ApiResponse>(`/posts/${postId}/view`, {
    method: 'POST',
...
export function recordLoopPlaybackTelemetry(postId: string, payload: Record<string, any> = {}): Promise<ApiResponse> {
  return apiRequest<ApiResponse>(`/posts/${postId}/loop-telemetry`, {
    method: 'POST',
    body: JSON.stringify(payload),
  })
}

export function createComment(postId: string, payload: FormData | Record<string, any>): Promise<{ comment: Comment }> {
  if (payload instanceof FormData) {
...
export function getPostLikes(
  ...
  return apiRequest<{ users: User[]; total: number }>(`/posts/${postId}/likes${query.toString() ? `?${query.toString()}` : ''}`)
}

export function getPostInsights(postId: string, days = 7): Promise<any> {
  const query = new URLSearchParams()
  if (days) {
...
export function togglePostLike(postId: string, recommendation: any = null): Promise<{ liked: boolean; likesCount: number }> {
  return apiRequest<{ liked: boolean; likesCount: number }>(`/posts/${postId}/like`, {
    method: 'POST',
    body: JSON.stringify(recommendation ? { recommendation } : {}),
  })
}

export function togglePostSave(postId: string, recommendation: any = null): Promise<{ saved: boolean; savesCount?: number }> {
  return apiRequest<{ saved: boolean; savesCount?: number }>(`/posts/${postId}/save`, {
    method: 'POST',
    body: JSON.stringify(recommendation ? { recommendation } : {}),
  })
}

export function togglePostShare(postId: string, recommendation: any = null): Promise<{ shared: boolean; sharesCount: number }> {
  return apiRequest<{ shared: boolean; sharesCount: number }>(`/posts/${postId}/share`, {
    method: 'POST',
    body: JSON.stringify(recommendation ? { recommendation } : {}),
  })
}

export function markPostNotInterested(postId: string, recommendation: any = null): Promise<ApiResponse> {
  return apiRequest<ApiResponse>(`/posts/${postId}/not-interested`, {
    method: 'POST',
    body: JSON.stringify(recommendation ? { recommendation } : {}),
  })
}

export function togglePostArchive(postId: string): Promise<{ isArchived: boolean }> {
  return apiRequest<{ isArchived: boolean }>(`/posts/${postId}/archive`, {
    method: 'POST',
  })
}

export function deletePost(postId: string): Promise<ApiResponse> {
  return apiRequest<ApiResponse>(`/posts/${postId}`, {
    method: 'DELETE',
  })
}

export function updatePost(postId: string, payload: Record<string, any>): Promise<{ post: Post }> {
  return apiRequest<{ post: Post }>(`/posts/${postId}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
...
export function toggleCommentLike(commentId: string): Promise<{ liked: boolean; likesCount: number }> {
  return apiRequest<{ liked: boolean; likesCount: number }>(`/posts/comments/${commentId}/like`, {
    method: 'POST',
  })
}

export function toggleCommentSave(commentId: string): Promise<{ saved: boolean }> {
  return apiRequest<{ saved: boolean }>(`/posts/comments/${commentId}/save`, {
    method: 'POST',
  })
}

export function toggleCommentShare(commentId: string): Promise<{ shared: boolean }> {
  return apiRequest<{ shared: boolean }>(`/posts/comments/${commentId}/share`, {
    method: 'POST',
  })
}

export function updateComment(commentId: string, payload: { text: string }): Promise<{ comment: Comment }> {
  return apiRequest<{ comment: Comment }>(`/posts/comments/${commentId}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
}

export function deleteComment(commentId: string): Promise<ApiResponse> {
  return apiRequest<ApiResponse>(`/posts/comments/${commentId}`, {
    method: 'DELETE',
  })
}
