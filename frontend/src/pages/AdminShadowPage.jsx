...
export default function AdminShadowPage() {
    ...
    getAdminShadowChats({ type: 'all' })
      .then((res) => {
        if (res?.chats) {
          ...
          if (!selectedChatKey && res.chats.length > 0 && activeTab === 'direct' && window.innerWidth >= 1024) {
            const first = res.chats[0]
            handleTabChange(first.type === 'room' ? 'rooms' : 'direct', first.chatKey || first.id)
          }
        }
      })
  ...
  // Handle shadow conversation delete
  const handleConfirmDeleteChat = async () => {
    if (!deleteTargetChat) return
    const targetKey = deleteTargetChat.chatKey || deleteTargetChat.id
    setIsDeletingChat(true)
    try {
      await deleteAdminShadowChat(targetKey, deleteChatReason)
      setChats((prev) => prev.filter((c) => (c.chatKey || c.id) !== targetKey))
      if (selectedChatKey === targetKey) {
        setMessages([])
      ...
      alert(err?.message || 'Gölge sohbeti silinirken hata oluştu.')
    } finally {
  ...
  return (
      ...
      <div className="w-full h-full flex flex-col min-h-0">
        ...
        <div className="rounded-none min-[681px]:rounded-md border-x-0 min-[681px]:border border-slate-200/90 bg-white shadow-sm min-[681px]:shadow-md h-[100dvh] min-[681px]:h-[calc(100dvh-32px)] min-h-[450px] flex overflow-hidden w-full">
            ...
            >
                ...
                {isSearchOpen ? (
                  <div className="flex items-center gap-1.5 w-full">
                    <div className="relative flex-1 min-w-0">
                      ...
                      />
                      <svg className="absolute left-2.5 top-2.5 size-3.5 text-blue-500 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      <span className="font-bold text-xs sm:text-sm text-slate-900 tracking-tight truncate">Sohbet Akışları</span>
                      ...
                      <button
                        ...
                        title="Gölge Sohbetlerinde Ara"
                        aria-label="Gölge Sohbetlerinde Ara"
                      ...
                      {/* Silinen Filtresi Butonu */}
                      <button
                        type="button"
                        onClick={() => setSplitDeletedOnly(!splitDeletedOnly)}
                        className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold border transition cursor-pointer shrink-0 ${
                          splitDeletedOnly
                            ? 'bg-rose-100 text-rose-700 border-rose-300 shadow-2xs'
                            : 'bg-white/80 text-slate-500 border-slate-200/90 hover:text-rose-600 hover:bg-rose-50/60'
                        }`}
                        title="Sadece silinen gölge sohbetleri göster"
                      >
                        Silinen
                      </button>
                    </div>
              ...
              {/* Filtre Kontrolleri: Buton Takımı (Sohbetler, Odalar, Aramalar, Medya) */}
                        ...
                        <div className="flex items-start justify-between gap-1.5 min-w-0">
                          ...
                          <div className="flex flex-col items-end shrink-0 -mt-1 -mr-1">
                            <button
                              ...
                              title="Gölge Sohbetini Kalıcı Olarak Sil"
    ...
    {/* 3. LIGHTBOX MEDIA MODAL */}
      ...
      {/* GÖLGE SOHBETİ KALICI SİLME ONAY MODALI */}
      {deleteTargetChat && (
        <div
          role="dialog"
          aria-modal="true"
            ...
            <div className="flex items-center gap-3 pb-3 border-b border-slate-100 mb-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">
                  {deleteTargetChat.type === 'room' ? 'Gölge Odasını Kalıcı Olarak Sil' : 'Gölge Sohbetini Kalıcı Olarak Sil'}
                </h3>
                <p className="text-xs text-slate-500">Bu işlem veritabanından tüm verileri tamamen temizler</p>
              </div>
            </div>

            <div className="p-3 rounded-2xl bg-rose-50/60 border border-rose-200/80 mb-4 text-xs text-rose-900 leading-relaxed">
              <strong>Dikkat:</strong> Bu gölge {deleteTargetChat.type === 'room' ? 'odasını' : 'sohbetini'} sildiğinizde; konuya ait <strong>tüm anonim mesajlar ({deleteTargetChat.totalMessages || 0} mesaj)</strong>, paylaşılan medya dosyaları, arama kayıtları ve bildirimler platformdan kalıcı olarak silinecektir. Bu işlem <u>kesinlikle geri alınamaz</u>.
            </div>

            <div className="mb-4 p-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-xs">
              <div className="flex items-center gap-1.5 font-bold text-slate-800 truncate mb-0.5">
                <span className="px-1.5 py-0.2 rounded text-[10px] bg-slate-200 text-slate-700">
                  {deleteTargetChat.type === 'room' ? 'Oda' : '1:1'}
                </span>
                <span className="truncate">{deleteTargetChat.title}</span>
              </div>
              <div className="text-slate-500 text-[11px]">
                Toplam {deleteTargetChat.totalMessages || 0} mesaj · Son aktivite: {formatDate(deleteTargetChat.lastMessageAt)}
              </div>
            </div>
